turtles-own [opinion]
globals [o1 o2 probab delta opponents h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 h13 h14 h15 h16 h17 h18 h19 h20 h21 i histogram-list]

to setup
  clear-all
  if file-exists? "evolution.txt" [file-delete "evolution.txt"]
  file-open "evolution.txt"
  create-turtles NumberAgents
  ask turtles [
    ;; move each turtle to a random point
    set opinion (random-float 2) - 1.0
...
This mechanism is well known in the literature, and in the absence of the tolerance thresholds (typical for the \textbf{bounded confidence models}) and with full connectivity, it leads to a convergence of the opinions. With the assumed symmetric initial distribution, should the `opinion exchange' be the only mechanism (e.g. for _p=0_), the resulting opinion distribution should correspond to all agents with $x<sub>i</sub>=0$ opinion. The factor $\mu$ determines the speed of the convergence of opinions; in the simulations we use relatively small values to ensure smoothness of the system evolution.
...
_x<sub>i</sub> \rightarrow x<sub>i</sub> + Delta<sub>i</sub>_,
 
where

_Delta<sub>i</sub> = d x<sub>i</sub> (1-|x<sub>i</sub>|) N<sub>OPP</sub>/N<sub>TOT</sub>_.

Here the parameter $\sigma$ determines the speed of the repulsion from the opposing views, and the ratio $N_{OPP}/N_{TOT}$ is the ratio of the number of agent $i$ opponents in the whole population. The repulsive correction $\Delta_i$ shifts the agent's opinion away from the opponents, and is intended to correspond to the repulsion people feel when they find or discuss descriptions of `hideous practices of the opposing camp' when reading media supporting their own views or when discussing with other people supporting their own opinions.
